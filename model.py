...
class RNN_ENCODER(nn.Module):
    ...
    def define_module(self):
        self.fc = nn.Sequential(nn.Linear(self.ntoken, self.ninput),
                                  nn.LeakyReLU(),
                                  nn.Linear(self.ntoken, self.ninput),
                                  nn.Tanh())
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        ...
        noise_reduced_captions = self.fc(captions)
        emb = self.drop(self.encoder(noise_reduced_captions))
        #
